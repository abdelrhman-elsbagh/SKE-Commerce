@extends('layouts.vertical', ['page_title' => 'Fee Groups'])
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="page-title-box">
                    <h4 class="page-title">Fee Groups</h4>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <a href="{{ route('fee_groups.create') }}" class="btn btn-primary mb-3">Create Fee Group</a>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Are you sure you want to delete this fee group?
                </div>
...
@section('admin-script')
    <script>
        $(document).ready(function() {
            var feeGroupIdToDelete;

            // Ensure click event handler is attached only once using event delegation
            $(document).on('click', '.btn-delete', function() {
                feeGroupIdToDelete = $(this).data('id');
                $('#deleteModal').modal('show');
            });

            $('#confirmDelete').on('click', function() {
                $.ajax({
                    url: '{{ route('fee_groups.index') }}/' + feeGroupIdToDelete,
                    type: 'POST',
                    data: {
                        _method: 'DELETE',
                        _token: '{{ csrf_token() }}'
                    ...
                    success: function(result) {
                        ...
                        $('#fee-group-' + feeGroupIdToDelete).remove();
                        // Show success message
                        $.toast().reset('all'); // Reset previous toasts
                        $.toast({
                            heading: 'Success',
                            text: 'Fee Group deleted successfully.',
                            icon: 'success',
                            loader: true,
                            loaderBg: '#f96868',
                            position: 'top-right',
                    ...
                    error: function(err) {
                        ...
                        $.toast({
                            heading: 'Error',
                            text: 'An error occurred while deleting the fee group.',
                            icon: 'error',
                            loader: true,
                            loaderBg: '#f96868',
                            position: 'top-right',
